export function ButtonPrimary({ children, onClick, className }: { children: React.ReactNode, onClick?: () => void, className?: string }) {
    return <button className={` max-h-10 bg-[var(--brand-primary)]  justify-center rounded-md border-2 border-[var(--brand-primary)] p-4 flex items-center gap-2 hover:bg-[var(--brand-secondary)] text-white ${className}`} onClick={onClick}>
        {children}
    </button>;
}
