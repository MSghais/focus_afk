...
export function ButtonSecondary({ children, onClick }: { children: React.ReactNode, onClick: () => void }) {
    return <button className="bg-[var(--brand-secondary)] rounded-md border-2 border-[var(--brand-secondary)] p-4 flex items-center gap-2 hover:bg-[var(--brand-primary)]" onClick={onClick}>
        {children}
    </button>;
}