...
import { notesSchema, UpdateNotesInput, GetNotesInput } from '../../validations/notes.validation';
import { CreateMentorInput, GetMessagesInput   } from '../../validations/mentor.validation';
dotenv.config();
...
async function notesRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/notes', {
    ...
    try {
      ...
      const mentor = await fastify.prisma.notes.create({
        data: {
          ...mentorData,
          userId,
        },
  ...
  fastify.get('/notes', {
    ...
    try {
      ...
      const notes = await fastify.prisma.mentor.findMany({
  ...
  fastify.get('/messages', {
    ...
    try {
      const userId = request.user.id;
      const { mentorId, limit, offset } = request.query as GetNotesInput;
